...
_KEYWORDS = ["収支", "経営費", "所得", "総収入", "経営収支"]


def find_balance_pages(pages: list[PDFPage], llm_runtime) -> list[int]:
    ...
    response = llm_runtime.request_json(
        system_prompt="Find pages that describe management balance and cost information.",
        user_prompt=(
            "From the candidate pages, select pages that contain balance information.\n"
