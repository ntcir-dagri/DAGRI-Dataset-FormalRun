...
_KEYWORDS = ["作業期間", "月/旬", "ガント", "作業暦", "栽培スケジュール"]


def find_work_schedule_pages(
    pages: list[PDFPage],
    llm_runtime,
    crop_names: list[str],
) -> list[int]:
    ...
    response = llm_runtime.request_json(
        system_prompt="Find pages describing crop-specific work schedule and gantt chart.",
        user_prompt=(
            f"Crop names: {crop_names}\n"
            "Select pages that contain work schedule or gantt chart per crop.\n"
